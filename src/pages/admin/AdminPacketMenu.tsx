...
export default function AdminPacketMenu({
  navigate,
  currentPage,
}: AuthPageProps) {
  ...
  const columns = [
    ...
    { accessorKey: "product.name", header: "Nama" },
    {
      accessorKey: "price",
      header: "Harga",
      cell: ({ row }) => {
        return (
          <span>
            ...
            {new Intl.NumberFormat("id-ID", {
              currency: "IDR",
            }).format(row?.original?.product?.price ?? 0)}
          </span>
        );
      },
    },
  ...
  const fetchDatatables = () => {
    ...
    useAxios
      .get(
        "/packet-product/datatables/" + extractParams[extractParams.length - 1],
        {
          params: {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    useAxios
      .post(
        "/packet-product/" + extractParams[extractParams.length - 1],
        formData,
        {
          headers: {
            Authorization: `Bearer ${currentUser?.access_token ?? ""}`,
